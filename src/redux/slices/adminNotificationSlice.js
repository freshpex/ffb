  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAdminNotifications.fulfilled, (state, action) => {
        ...
        if (action.payload) {
          ...
          if (Array.isArray(action.payload)) {
            ...
            console.error('Unexpected notification payload structure:', action.payload);
            state.notifications = [];
            state.unreadCount = 0;
          }
        } else {
      ...
      .addCase(markNotificationAsRead.fulfilled, (state, action) => {
        ...
        const index = state.notifications.findIndex(n => n._id === action.payload.id || n.id === action.payload.id);
        if (index !== -1) {
...
// Export selectors
export const selectAdminNotifications = state => state.adminNotifications.notifications;
export const selectUnreadCount = state => state.adminNotifications.unreadCount;
export const selectAdminNotificationStatus = state => state.adminNotifications.status;
export const selectAdminNotificationError = state => state.adminNotifications.error;
export const selectAdminNotificationActionStatus = state => state.adminNotifications.actionStatus;
