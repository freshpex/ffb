  ...
  cardTypes: [
    {
      ...
      features: ["Online purchases", "Digital wallets", "No physical card"],
    },
    {
      id: "standard-debit",
  ...
  async (cardId, { rejectWithValue }) => {
    try {
      const response = await apiService.post(
        `/admin/atm-cards/${cardId}/approve`,
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to approve card request",
      );
    }
  },
);
...
export const rejectCardRequest = createAsyncThunk(
  ...
  async ({ cardId, reason }, { rejectWithValue }) => {
    try {
      const response = await apiService.post(
        `/admin/atm-cards/${cardId}/reject`,
        { reason },
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to reject card request",
      );
    }
  },
);
