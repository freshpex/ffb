...
export const requestCard = createAsyncThunk(
  ...
  async (cardData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to request card",
      );
    }
  },
);
...
export const cancelCardRequest = createAsyncThunk(
  ...
  async (requestId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Failed to cancel card request",
      );
    }
  },
);
  ...
  async ({ cardId, limits }, { rejectWithValue }) => {
    try {
      const response = await apiClient.put(
        `/atm-cards/${cardId}/limits`,
        limits,
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to update card limits",
      );
    }
  },
);
...
export const fetchCardTransactions = createAsyncThunk(
  ...
  async (
    { cardId, page = 1, limit = 10, filters = {} },
    { rejectWithValue },
  ) => {
    try {
      ...
      const response = await apiClient.get(
        `/atm-cards/${cardId}/transactions?${queryParams}`,
      );
      return { cardId, data: response.data };
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch card transactions",
      );
    }
  },
);
...
export const fundCardFromBalance = createAsyncThunk(
  ...
  async ({ cardId, amount }, { rejectWithValue }) => {
    try {
      const response = await apiClient.post(`/atm-cards/${cardId}/fund`, {
        amount,
      });
      return { cardId, data: response.data };
    } catch (error) {
    ...
    builder
      ...
      .addCase(fetchCards.fulfilled, (state, action) => {
        ...
        if (action.payload.data) {
          ...
          state.cardRequests = allCards.filter((card) =>
            ["pending", "rejected"].includes(card.status),
          );
        } else {
          state.cards = [];
          state.cardRequests = [];
      ...
      .addCase(cancelCardRequest.fulfilled, (state, action) => {
        state.status = "succeeded";
        const index = state.cardRequests.findIndex(
          (request) => request._id === action.payload.data._id,
        );
        if (index !== -1) {
          state.cardRequests[index] = {
            ...state.cardRequests[index],
            status: "cancelled",
          };
        }
      ...
      .addCase(createCardTransaction.fulfilled, (state, action) => {
        ...
        const cardIndex = state.cards.findIndex(
          (card) => card._id === cardId || card.id === cardId,
        );
...
export const selectTransactionStatus = (state) =>
  state.atmCards.transactionStatus;
export const selectTransactionError = (state) =>
  state.atmCards.transactionError;
