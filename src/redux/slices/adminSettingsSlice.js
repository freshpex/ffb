...
export const fetchSettingsByCategory = createAsyncThunk(
  ...
  async (category, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${API_URL}/admin/settings/category/${category}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      return rejectWithValue(error.message || 'Failed to fetch category settings');
    }
  }
);
    ...
    builder
      ...
      .addCase(updateSettings.fulfilled, (state, action) => {
        ...
        if (action.payload && action.payload.updated) {
          action.payload.updated.forEach(update => {
            const category = Object.keys(state.settings).find(cat => 
              state.settings[cat] && state.settings[cat][update.key]
            );
      ...
      .addCase(deleteSetting.fulfilled, (state, action) => {
        ...
        Object.keys(state.settings).forEach(category => {
          if (state.settings[category] && state.settings[category][keyToDelete]) {
...
export const selectSettingsByCategory = (state, category) => state.adminSettings.settings[category] || {};
...
export const selectAdminSettingsActionStatus = state => state.adminSettings.actionStatus;
