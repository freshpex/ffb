...
export const generateMockTradingPairs = () => {
  // Cryptocurrencies
  const cryptoBaseCurrencies = ['BTC', 'ETH', 'SOL', 'ADA', 'DOT', 'AVAX', 'MATIC', 'XRP', 'BNB', 'LINK'];
  ...
  // Commodities
  const commodities = ['GOLD', 'SILVER', 'OIL', 'NAT.GAS', 'COPPER', 'WHEAT', 'CORN', 'COFFEE'];
  
  // Forex pairs
  const forexPairs = ['EUR/USD', 'USD/JPY', 'GBP/USD', 'USD/CHF', 'AUD/USD', 'USD/CAD', 'NZD/USD', 'EUR/GBP'];
...
function getForexPrice(pair) {
  const forexPrices = {
    ...
    'USD/CHF': 0.90 + Math.random() * 0.02,
    'AUD/USD': 0.65 + Math.random() * 0.02,
    'USD/CAD': 1.35 + Math.random() * 0.03,
    'NZD/USD': 0.60 + Math.random() * 0.01,
 ...
 */
export const generateMockCandlesticks = (symbol, timeframe = '1h', limit = 100) => {
