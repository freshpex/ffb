...
const CardsList = ({ cards, onCardAction }) => {
  ...
  const handleFreezeCard = async (cardId) => {
    ...
    onCardAction('freeze', 'Card frozen successfully. You can unfreeze it anytime.');
  };
  
  const handleUnfreezeCard = async (cardId) => {
    await dispatch(unfreezeCard(cardId));
    setActionMenuOpen(null);
    onCardAction('unfreeze', 'Card unfrozen successfully. You can now use your card again.');
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {cards.map((card) => (
        ...
        >
          {/* Card Display */}
          <div className={`${cardDesigns[card.type]} p-6 relative overflow-hidden h-48`}>
            {/* Card Status Badge */}
            {card.frozen && (
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-gray-300 text-xs">{getCardTypeName(card.type)}</p>
              </div>
            ...
            <div className="mt-6">
              <div className="flex justify-between items-center">
                ...
                >
                  {showCardNumber[card.id] ? <FaEyeSlash size={14} /> : <FaEye size={14} />}
                </button>
              </div>
              <p className="text-white font-mono font-medium text-lg tracking-wider">
                {showCardNumber[card.id] ? card.cardNumber.replace(/\*{4}/g, '5678') : card.cardNumber}
              </p>
            </div>
          ...
          <div className="p-4">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-white text-lg font-semibold">${card.balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
