...
import { FaCog, FaTimes, FaCheck, FaExclamationTriangle, FaInfoCircle } from 'react-icons/fa';
...
const CardLimitsModal = ({ isOpen, onClose, card, onSuccess }) => {
  ...
  const validateLimits = () => {
    ...
    if (limits.daily > maxLimits[cardType].daily) {
      setError(`Maximum daily limit for this card type is $${maxLimits[cardType].daily.toLocaleString()}`);
      return false;
    }
    
    if (limits.monthly > maxLimits[cardType].monthly) {
      setError(`Maximum monthly limit for this card type is $${maxLimits[cardType].monthly.toLocaleString()}`);
      return false;
    }
          ...
          <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div className="mb-6">
              <p className="text-gray-300 mb-4">
                Adjust your card spending limits to control your transactions. These limits apply to all purchases and withdrawals made with this card.
              </p>
              ...
              <div className="bg-gray-900/50 p-4 rounded-lg mb-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className={`text-sm px-2 py-1 rounded-full ${
                    card.frozen ? 'bg-red-900/30 text-red-400 border border-red-700' : 'bg-green-900/30 text-green-400 border border-green-700'
                  }`}>
              ...
              <div className="space-y-4 mb-4">
                <div>
                  <label className="flex justify-between">
                    <span className="block text-sm font-medium text-gray-400 mb-1">Daily Limit</span>
                    <span className="text-sm text-gray-500">Current: ${card.limits.daily.toLocaleString()}</span>
                  </label>
                  ...
                  <div className="mt-1 h-1 bg-gray-600 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(card.limits.dailyUsed / limits.daily) * 100}%` }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>Used: ${card.limits.dailyUsed.toLocaleString()}</span>
                    <span>Available: ${(limits.daily - card.limits.dailyUsed).toLocaleString()}</span>
                  </div>
                </div>
                
                <div>
                  <label className="flex justify-between">
                    <span className="block text-sm font-medium text-gray-400 mb-1">Monthly Limit</span>
                    <span className="text-sm text-gray-500">Current: ${card.limits.monthly.toLocaleString()}</span>
                  </label>
                  <div className="flex items-center">
                    <span className="bg-gray-700 px-3 py-2 rounded-l-lg text-gray-400">$</span>
                    <input
                  ...
                  <div className="mt-1 h-1 bg-gray-600 rounded-full overflow-hidden">
                    <div 
                      className="bg-primary-400 h-full"
                      style={{ width: `${(card.limits.monthlyUsed / limits.monthly) * 100}%` }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>Used: ${card.limits.monthlyUsed.toLocaleString()}</span>
                    <span>Available: ${(limits.monthly - card.limits.monthlyUsed).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-900/20 border border-blue-700 p-3 rounded-lg text-sm text-blue-400 flex items-start mb-4">
                ...
                <div>
                  <p className="mb-1">Limit changes take effect immediately. If you lower your limits below your current usage, new transactions may be declined until usage falls below the new limit.</p>
                </div>
              </div>
            </div>
          </div>
