...
const InvestmentPlans = () => {
  ...
  const handleCancelInvestment = async (investmentId) => {
    if (
      window.confirm(
        "Are you sure you want to cancel this investment? You will not receive any returns.",
      )
    ) {
      try {
  ...
  const handleWithdrawInvestment = async (investmentId) => {
    if (
      window.confirm(
        "Are you sure you want to withdraw from this investment? Returns will be pro-rated based on time invested.",
      )
    ) {
      try {
  ...
  const filteredAndSortedPlans = [...plans]
    ...
    .filter((plan) => {
      ...
      switch (durationFilter) {
        ...
        case "medium":
          return plan.duration > 30 && plan.duration <= 90;
        case "long":
        ...
        <Tabs
          tabs={[
            ...
            },
            {
              ...
              count: historyInvestments?.length,
            },
          ]}
          activeTab={activeTab}
                  ...
                  <div className="space-y-4">
                    {activeInvestments.map((investment) => (
                      <InvestmentHistoryCard
                        ...
                        onWithdraw={() =>
                          handleWithdrawInvestment(investment._id)
                        }
                      />
                    ))}
                  </div>
