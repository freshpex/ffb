...
const Withdraw = () => {
  ...
  const validateForm = () => {
    ...
    } else if (activeMethod?.id === 'paypal' && !/\S+@\S+\.\S+/.test(formData.paypalEmail)) {
      ...
      >
        ...
        >
          <div className="mb-6 text-center">
            ...
            <p className="text-gray-400">Your withdrawal request has been successfully submitted and is being processed.</p>
          </div>
          
          <div className="space-y-4 mb-6">
            <div className="flex justify-between py-2 border-b border-gray-700">
              <span className="text-gray-400">Amount:</span>
              <span className="text-white font-medium">${Math.abs(pendingWithdrawal.amount)}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-700">
              <span className="text-gray-400">Fee:</span>
              <span className="text-white font-medium">${pendingWithdrawal.fee || calculateFee(Math.abs(pendingWithdrawal.amount))}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-700">
              <span className="text-gray-400">Method:</span>
              <span className="text-white font-medium">{pendingWithdrawal.method}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-700">
              <span className="text-gray-400">Status:</span>
              <span className="text-yellow-400 font-medium">{pendingWithdrawal.status || "Pending"}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-700">
              <span className="text-gray-400">Date:</span>
              <span className="text-white font-medium">{new Date(pendingWithdrawal.createdAt || Date.now()).toLocaleString()}</span>
            </div>
          </div>
      ...
      >
        ...
        <div className="bg-gray-900 rounded-lg shadow-lg overflow-hidden">
          <div className="p-6">
            ...
            {/* Withdrawal Methods */}
            <div className="mb-6">
              <h2 className="text-lg font-medium text-white mb-4">Select Withdrawal Method</h2>
            ...
            {activeMethod && (
              ...
              >
                ...
                {activeMethod.id === 'cryptocurrency' && formData.cryptoType && (
                  <FormInput
                ...
                {formData.amount && !isNaN(formData.amount) && parseFloat(formData.amount) > 0 && (
                  <div className="bg-gray-800 p-4 rounded-lg my-6">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span className="text-gray-400">Amount:</span>
                        <span className="text-white">${parseFloat(formData.amount).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Fee (1%):</span>
                        <span className="text-white">${calculateFee(formData.amount).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between font-medium">
                        <span className="text-gray-300">Total:</span>
                        <span className="text-white">${calculateTotal(formData.amount).toFixed(2)}</span>
                      </div>
