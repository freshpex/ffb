...
import { FaWallet, FaChevronUp, FaChevronDown, FaExchangeAlt, FaArrowRight, FaSpinner } from 'react-icons/fa';
import {
  fetchPortfolio,
  selectPortfolio,
  setSelectedSymbol
...
const AssetCards = ({
  ...
  const getAssetIcon = (symbol) => {
    ...
    switch (assetSymbol) {
      case 'BTC':
        return <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white font-bold">₿</div>;
      case 'ETH':
        return <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold">€</div>;
      case 'USDT':
        return <div className="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white font-bold">$</div>;
      default:
        return <div className="w-8 h-8 rounded-full bg-gray-500 flex items-center justify-center text-white font-bold">{assetSymbol.charAt(0)}</div>;
    }
  };
  ...
  const getAssets = () => {
    ...
    for (const [asset, balance] of Object.entries(balances)) {
      if (asset !== 'USDT' && asset !== 'USD' && !assets.find(a => a.asset === asset)) {
    ...
    return (
      <div className={`bg-gray-800 rounded-lg p-4 ${className}`}>
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-gray-400 mb-4">Your assets will appear here once you deposit or trade.</p>
          <button 
                ...
                <div className="flex-grow">
                  <div className="flex justify-between items-center mb-1">
                    <p className="font-medium text-white">
                      {asset.asset}
                      {asset.isStablecoin && <span className="ml-1 text-xs text-gray-400">(Stable)</span>}
                    </p>
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    {asset.pnl !== 0 && (
                      <span className={`flex items-center ${asset.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {asset.pnl >= 0 ? <FaChevronUp size={10} className="mr-1" /> : <FaChevronDown size={10} className="mr-1" />}
            ...
            <div className="flex items-start">
              ...
              <div className="flex-grow">
                <div className="flex justify-between items-center mb-2">
                  <p className="font-medium text-white text-lg">{asset.asset}</p>
                  {asset.pnl !== 0 && (
                    <span className={`text-xs font-medium px-2 py-1 rounded flex items-center ${
                      asset.pnl >= 0 ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'
                    }`}>
                      {asset.pnl >= 0 ? <FaChevronUp size={8} className="mr-1" /> : <FaChevronDown size={8} className="mr-1" />}
                      {Math.abs(asset.pnlPercentage).toFixed(2)}%
                    </span>
                  )}
                </div>
                ...
                <p className="text-2xl font-semibold text-white mb-2">
                  {formatCurrency(asset.balance)} <span className="text-sm font-normal text-gray-400">{asset.asset}</span>
                </p>
