    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const kycResponse = await dispatch(
          fetchKycRequests({ page: 1, limit: 50 }),
        ).unwrap();

        // Fetch support tickets data
        const supportResponse = await dispatch(
          fetchSupportTickets({ page: 1, limit: 50 }),
        ).unwrap();
        ...
        const userStats = {
          ...
          pending: (usersResponse?.users || []).filter(
        ...
        const transactionStats = {
          ...
          completed: (transactionsResponse?.transactions || []).filter(
            (t) => t?.status === "completed",
          ).length,
          failed: (transactionsResponse?.transactions || []).filter(
        ...
        const kycStats = {
          ...
          approved: (kycResponse?.kycRequests || []).filter(
            (k) => k?.status === "approved",
          ).length,
          rejected: (kycResponse?.kycRequests || []).filter(
            (k) => k?.status === "rejected",
          ).length,
        };
        ...
        const ticketStats = {
          ...
          resolved: (supportResponse?.supportTickets || []).filter(
        ...
        setRecentTransactions(
          (transactionsResponse?.transactions || []).slice(0, 5),
        );
                    ...
                    recentKycRequests.map((kyc) => (
                      <div key={kyc.id} className="px-6 py-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div>{renderStatusBadge(kyc.status)}</div>
                        </div>
              ...
              >
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead
                      className={darkMode ? "bg-gray-700/30" : "bg-gray-50"}
                    >
                      <tr>
