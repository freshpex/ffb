...
import { FaBell, FaCheck, FaCheckDouble, FaTrash, FaExclamationCircle, FaTimes } from 'react-icons/fa';
...
const NotificationItem = ({ notification, onRead, onDelete }) => {
  ...
  const getIcon = () => {
    switch (notification.type) {
      case 'success':
        return <div className="bg-green-700/20 p-2 rounded-full text-green-500"><FaCheck /></div>;
      case 'warning':
        return <div className="bg-yellow-700/20 p-2 rounded-full text-yellow-500"><FaExclamationCircle /></div>;
      case 'error':
        return <div className="bg-red-700/20 p-2 rounded-full text-red-500"><FaTimes /></div>;
      case 'transaction':
        return <div className="bg-blue-700/20 p-2 rounded-full text-blue-500"><FaCheck /></div>;
      case 'kyc':
        return <div className="bg-purple-700/20 p-2 rounded-full text-purple-500"><FaCheck /></div>;
      case 'support':
        return <div className="bg-teal-700/20 p-2 rounded-full text-teal-500"><FaCheck /></div>;
      default:
        return <div className="bg-gray-700/20 p-2 rounded-full text-gray-500"><FaBell /></div>;
    }
  };
    ...
    >
      ...
      <div className="flex-grow">
        <div className="flex justify-between">
          <h4 className={`font-medium ${!notification.read ? 'text-blue-400' : 'text-gray-300'}`}>
            {notification.title}
          </h4>
    ...
    <div className="fixed inset-0 z-30 bg-black/50 overflow-auto">
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto">
          ...
          {status === 'succeeded' && notifications.map((notification) => (
            <NotificationItem 
