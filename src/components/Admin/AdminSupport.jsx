...
import { fetchSupportTickets, selectSupportTickets, selectSupportPagination, selectSupportStatus } from '../../redux/slices/adminSupportSlice';
  ...
  return (
    <PageTransition>
      <div>
        ...
        {isLoading ? (
          <ComponentLoader height="400px" message="Loading support tickets..." />
        ) : (
          <div>
                      ...
                      tickets.map((ticket) => (
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <Link to={`/admin/support/${ticket.id}`} className="block">
                              <div className="text-sm font-medium truncate max-w-xs">{ticket.subject}</div>
                              <div className="text-xs text-gray-500 dark:text-gray-400">
                                #{ticket.ticketNumber} • {timeElapsed(ticket.createdAt)}
                              </div>
                            </Link>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <div className={`h-8 w-8 rounded-full mr-2 flex items-center justify-center ${
                                ...
                                <FaUser className={`h-4 w-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`} />
                              </div>
                              <div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            {timeElapsed(ticket.lastUpdated || ticket.createdAt)}
                          </td>
            ...
            <div className={`mt-6 p-4 rounded-lg ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-xs uppercase font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Open Tickets</p>
                ...
                <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-xs uppercase font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>In Progress</p>
                ...
                <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-xs uppercase font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Resolved</p>
                ...
                <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-xs uppercase font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Urgent Priority</p>
                  <p className={`mt-1 text-xl font-semibold text-red-500`}>
                    {tickets.filter(t => t.priority === 'urgent' && (t.status === 'open' || t.status === 'in_progress')).length}
                  </p>
                </div>
              </div>
            </div>
