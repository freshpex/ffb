...
const TransactionAnalytics = () => {
  ...
  const status = useSelector((state) =>
    selectAnalyticsStatus(state, "transactions"),
  );
  const error = useSelector((state) =>
    selectAnalyticsError(state, "transactions"),
  );
  ...
  const prepareStatusDistribution = () => {
    if (
      ...
      !transactionData.distribution.byStatus
    ) {
      return [];
    }
  ...
  const prepareTypeDistribution = () => {
    if (
      !transactionData ||
      !transactionData.distribution ||
      !transactionData.distribution.byType
    ) {
      return [];
    }
      ...
      <div className="bg-gray-800 rounded-lg shadow-md p-4 md:p-6">
        <div className="bg-red-900/30 border-l-4 border-red-500 p-4">
          <p className="text-red-400">
            Error loading transaction analytics: {error}
          </p>
          <button
                  ...
                  >
                    {prepareStatusDistribution().map((entry) => (
                      <Cell
                        key={entry.name}
                        fill={
                          STATUS_COLORS[entry.name.toLowerCase()] || "#A3A3A3"
                        }
                      />
                    ))}
                  </Pie>
                  ...
                  >
                    {prepareTypeDistribution().map((entry) => (
                      <Cell
                        key={entry.name}
                        fill={
                          TYPE_COLORS[entry.name.toLowerCase()] || "#A3A3A3"
                        }
                      />
                    ))}
                  </Pie>
                      ...
                      <tr key={type.name}>
                        <td className="py-2 text-sm whitespace-nowrap text-gray-300 flex items-center">
                          <div
                            ...
                            style={{
                              backgroundColor:
                                TYPE_COLORS[type.name.toLowerCase()] ||
                                "#A3A3A3",
                            }}
                          ></div>
                          {type.name}
